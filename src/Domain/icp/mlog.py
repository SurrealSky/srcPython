...
def create_logger(log_dir, log_filename, backup_count, log_level=logging.DEBUG):
    """
    ...
    config.log.save_log = False  # 确保日志保存功能启用
    if config.log.save_log:
        # 确保日志目录存在
        print(f"日志目录 {log_dir} 不存在，正在创建...")
    ...
    config.log.output_console = True  # 确保控制台输出功能启用
...
logger.info("日志记录器已创建，日志级别为: %s", logging.getLevelName(logger.level))